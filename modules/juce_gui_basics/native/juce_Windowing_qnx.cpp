...
namespace
{
    ...
    // Dirty-region software present: render, blit and post only what changed, and
    // reuse a persistent backing image rather than allocating and clearing a
    // full-window ARGB image every frame. This is the default path.
    //
    // Screen-allocated buffers are uncached on the targets tested, so the copy is
    // the dominant cost and confining it to the invalidated rectangle matters:
    // 16.7 -> 59.2 fps at 1280x1024 on a small-invalidation workload (RPi4).
    //
    // JUCE_QNX_FAST_PRESENT=0 selects the simpler full-window path instead, which
    // is kept as a reference implementation for A/B measurement and for bringing
    // up new QNX targets where the dirty-region path may need validating.
    ...
    public:
        ...
        void performAnyPendingRepaintsNow() override
        {
            ...
            // Reference path: re-render and post the whole window every frame.
        ...
        // Dirty-region-aware software repaint. See shouldUseFastQnxPresent().
