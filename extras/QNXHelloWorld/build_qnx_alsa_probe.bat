...
set BUILD_DIR=%ROOT%\build\qnx_alsa_probe\%TARGET_DIR%
...
q++ -V%TARGET% -std=gnu++17 -I"%BUILD_DIR%" ^
    ...
    -c "%ROOT%\extras\QNXHelloWorld\Source\ALSAProbe.cpp" ^
    -o "%BUILD_DIR%\ALSAProbe.o" || exit /b 1

q++ -V%TARGET% ^
    "%BUILD_DIR%\ALSAProbe.o" ^
    -lasound ^
    -o "%BUILD_DIR%\QNXALSAProbe" || exit /b 1

echo Built: %BUILD_DIR%\QNXALSAProbe
