...
public:
    ...
    AndroidProjectExporter (Project& p, const ValueTree& t)
        : ProjectExporter (p, t),
          ...
          androidUseSharedSTL                  (settings, Ids::androidUseSharedSTL,                  getUndoManager(), false),
...
private:
    void writeCmakeFile (const File& file) const
    {
        build_tools::writeStreamToFile (file, [&] (MemoryOutputStream& mo)
        {
            ...
            mo << androidCustomCMakeListsAdditions.get().toString() << newLine;
        });
    }
    ...
    void createManifestExporterProperties (PropertyListBuilder& props)
    {
        ...
        props.add (new ChoicePropertyComponent (androidUseSharedSTL, "STL Shared Library Type"),
                   "Enable this to build against the Shared STL rather than default Static STL");
        props.add (new TextPropertyComponent (androidCustomCMakeListsAdditions, "Custom CMakeLists.txt Additions", 8192, true),
                   "You can use this to append custom additions to the end of the CMakeLists.txt build script");
        props.add (new TextPropertyComponent (androidCustomJuceAppImports, "Custom JuceApp.java imports", 8192, true),
                   "You could theoretically use this to add custom imports to JuceApp.java");
        props.add (new TextPropertyComponent (androidCustomJuceAppOnCreateAdditions, "Custom JuceApp.java onCreate() additions", 8192, true),
                   "You could theoretically use this to add custom additions to JuceApp.java's onCreate() method");
    }
    ...
    StringArray getCmakeDefinitions() const
    {
        ...
        String stlType;
        androidUseSharedSTL.get() ? stlType = "shared" : stlType = "static";
        cmakeArgs.add (String ("\"-DANDROID_STL=") + (isClang ? "c++_" + stlType : "gnustl_" + stlType) + "\"");
