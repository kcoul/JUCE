...
TARGET_DIR="${TARGET//,/_}"
BUILD_DIR="$ROOT/build/qnx_desktop_window_demo/$TARGET_DIR"
GENERATED_HEADER="$BUILD_DIR/GeneratedBuildVersion.h"

mkdir -p "$BUILD_DIR"

cmake -DROOT="$ROOT" -DOUTPUT_HEADER="$GENERATED_HEADER" -P "$ROOT/extras/QNXDesktopWindowDemo/cmake/GenerateBuildVersion.cmake"
...
q++ "${COMMON[@]}" -c "$ROOT/extras/QNXDesktopWindowDemo/Source/Main.cpp" -o "$BUILD_DIR/Main.o"
...
q++ "-V${TARGET}" \
  ...
  -o "$BUILD_DIR/JUCEQNXDesktopWindowDemo"
